...
interface KreamHistoryChartComponentProps {
    list: any[]
}

export default function KreamHistoryChartComponent({ list }: KreamHistoryChartComponentProps) {
                    ...
                    <TableBody sx={{ heigh: '50px', overflow: 'hidden' }}>
                        {list && list.map((item: any, index) => (
                            <StyledTableRow key={index} sx={item.isSnkrDunkOk && {
                                ...
                                <StyledTableCell align="right">{item?.option || 0}</StyledTableCell>
                                {/* <StyledTableCell align="right">{}</StyledTableCell> */}
                                <StyledTableCell align="right">{VND.format((item?.price * 103.8 / 100 + 3000 + 25000) * rateKor)}</StyledTableCell>
                                <StyledTableCell align="right">{item?.date_created.replaceAll('T', ' ').replaceAll('Z', ' ')
                                }</StyledTableCell>
                            </StyledTableRow>
                        ))}
                    </TableBody>
